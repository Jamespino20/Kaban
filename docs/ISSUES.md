...
- \[ ] Fraunces and PlusJarkartaSans fonts implemented as the standard font (layout.tsx + globals.css)
- \[ ] CSS updated with premium SaaS styles (rounded-xl everywhere, proper font hierarchy)
...
- \[ ] Add username field to the profile editing, then randomize the member code numbers. Do not attach the membercode to the username, then implement this naming convention: \[username]\[slug]\[role_initials]

#### Homepage

- \[ ] Widen the navbar (now uses max-w-\[100rem])
- \[ ] Reroute Get Started to have the Tenant Onboarding logic, removing it from the Agapay Platform's Contact Page
- \[ ] Live Map should be zoomable because tenants near each other are clipping against each other. Be more accurate to the mapping logic for closely-located tenants to avoid pin-clipping
...
- \[ ] Custom green circle scrollbar implemented (globals.css)
...
- \[ ] Custom green circle scrollbar implemented
...
- \[ ] Custom green circle scrollbar implemented
- \[ ] Sidebar indentation fixed (px-4 alignment)
- \[ ] Contrast-aware sidebar text implemented
- \[ ] Three-dot actions added to header dropdown
...
- \[ ] Inactivity timeout/warning modal implemented (idle-session-timer.tsx)
...
- \[ ] Better categorize modules according to use hierarchy and frequency (already categorized: Core Operations, Capital, Members, Loan Operations, Storefront, Support \& Analytics, System)
- \[ ] Settings and Audit Logs at the very bottom (System category)
- \[ ] Operators should not have an interest tier of their own \[Overview]
- \[ ] Payment Intake and Capital Top-Up ledger errors fixed (postLedgerEntry uses account connect)
- \[ ] Capital Top-Ups from Members should be processed automatic (handled by the system)
- \[ ] Capital and Investments contains redundant KPIs (AnalyticsDashboardTab placeholder)
- \[ ] Member Management three-dot actions are functional (DropdownMenu with View Profile, Edit Details, Reset Password, Activity Log, Send Notification, Suspend/Activate/Deactivate)
...
- \[ ] Overview is fixed in the sidebar (px-4 alignment)
- \[ ] Wallet is fixed in the sidebar (px-4 alignment)
...
- \[ ] When the loan amount is low, Installments are always 0/0, which softlocks the user from paying at all (fixed: shows "—" instead of "0/0", schedule section always visible with helpful message)
- \[ ] Processed repayments not updating to paid fixed (verifySubmittedPayment now handles partial payments)
...
- \[ ] Support System's Survey should be more structured where the superadmin can add, edit, or remove questions in the survey.
- \[ ] Settings now includes profile image editing, account/personal information with form persistence, preferences, security details, TnC and other policies, account deactivation, download backup
