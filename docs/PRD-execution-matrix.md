...
| P0-02 | Data model baseline | NEW | Rebuild schema and policy constants around tenants, regions, roles, users, wallets, ledger entries, loans, repayments, documents, feedback, audit logs, reports, chats, notifications, and content. Use Section J as the implementation checklist. |
...
| --- | --- | --- | --- |
| DM-01 | Tenant regions | REBUILD | `TenantGroup` is the PRD parent region concept. Rename/code it to a clearer region-oriented domain name and keep tenant grouping, active tenant discovery, and Superadmin region creation support. |
| DM-02 | Tenants | REBUILD | Keep `Tenant` as the central tenant record, then verify/add fields for slug, active/availed/suspended/decommissioned/restored state, selected plan, member estimate, tenant contact details, tenant score, portfolio, last availment date, homepage content ownership, dashboard customization, and schema provisioning metadata. |
| DM-03 | Tenant schemas | VERIFY | Keep current schema-isolation approach from `getBranchPrisma`, migrations, and tenant creation flows. Add an explicit provisioning contract for new tenant schema creation, migration status, and recovery metadata. |
| DM-04 | Roles and RBAC | REBUILD | Standardize role values to Superadmin, Tenant Admin, Tenant Lender, Tenant Member at UI/domain level while preserving compatible enum values where needed. Add configurable RBAC templates and tenant dashboard feature toggles by plan. |
| DM-05 | Users and profiles | REBUILD | Keep `User` and `UserProfile` foundations. Add or verify PRD registration fields: gender, marital status, addresses, income/employment, business info, financial obligations, savings/assets, references, prefer-not-to-say handling, profile picture, theme preference, linked accounts, and bank/wallet accounts. Remove Mother's Maiden Name as redundant and out of scope. |
| DM-06 | Member tenant membership limit | REBUILD | Change policy from current max 3 memberships to PRD max 2 tenants per member and enforce it at registration/login/tenant membership creation boundaries. |
| DM-07 | Identity and documents | REBUILD | Keep `UserDocument` and verification status foundations. Verify/add document categories for valid government ID, selfie with ID, business permit, barangay residency certificate, proof of income, tenant application documents, loan attachments, payment proofs, and bulk verification metadata. |
| DM-08 | 2FA and security | VERIFY | Keep `TwoFactorAuth`, `TwoFactorToken`, session fields, and TOTP flow. Add data support for tenant/platform 2FA enforcement rules, session control, and security audit events. |
| DM-09 | Subscription plans | REBUILD | Keep subscription models if compatible. Seed/enforce Core P3,500/mo, Pro P6,500/mo, Enterprise P12,000/mo, and Sangay +P3,000/branch/mo with member limits, lender limits, feature flags, branch limits, billing cycles, invoices, and tenant plan state. |
| DM-10 | Tenant homepage customization | REBUILD | Keep existing branding/content foundations. Tenant/branch-specific homepages should use the same layout structure as the main homepage, with tenant-owned content and branding. Verify/add brand color, main color, accent color, logo, typography, hero content, mission/vision, values, calculator configs, starter FAQs/testimonials, announcements banner, contact content, and preview metadata. |
| DM-11 | Platform homepage content | REBUILD | Keep FAQ/testimonial workflow foundations. Add parent FAQ seasons, hide/show season state, bulk tick state, custom FAQ override behavior, tenant testimonial selection, tenant hide/show state, and platform-scoped moderation metadata. |
| DM-12 | Loan products | REBUILD | Keep `LoanProduct`. New tenants and branches start with an empty loan product set by default. PRD sample products (Agapay Sari-Sari, Agapay Negosyo, Agapay Paluwagan, Agapay Angat) should be optional templates only, not automatic seeds. Add/verify product ranges, special-case upper limit support, flat/diminishing interest model, cadence options, eligibility rules, and risk scoring modifiers. |
| DM-13 | Interest tiers | REBUILD | Keep interest tier enum shape if compatible, but rename/display PRD tiers: Gabay 5%, Bagong Sigla 4.5%, Kasapi 4%, Katuwang 3.5%, Kaagapay 3%. Default all new users to Gabay. |
| DM-14 | Loan policy constants | REBUILD | Replace stale constants with PRD values: loan amount P2,000-P1,000,000, weekly/bi-weekly/monthly cadence, P20 processing fee, P50 service fee, 1-2 guarantors, 25% guarantor liability, one active loan per user per tenant. |
| DM-15 | Penalty policy | REBUILD | Keep penalty helper structure where useful. Enforce PRD staircase: 2% for 1-3 days, 5% for 4-7 days, 8% for 8-14 days, 12% for 15+ days, capped at 20% of missed installment. |
| DM-16 | Loans | REBUILD | Keep `Loan`, `LoanSchedule`, `Payment`, and servicing foundations. Add/verify purpose, reference number, selected guarantors, loaner/lender linkage, admin approval chain, rejection reason, fund release state, payment option, full-payment discount support, and immutable transaction records. |
| DM-17 | Guarantorship | REBUILD | Keep `LoanGuarantee` if compatible. Add liability percentage, guarantor charge state, default-triggered liability records, revocation/reassignment hooks if later required, and notification/audit links. |
| DM-18 | Wallet and ledger | REBUILD | Keep `SavingsAccount`, `SavingsTransaction`, `TopUpRequest`, `LedgerAccount`, and `BusinessLedger` foundations. Add PRD deposit/withdrawal records, method selection, fees, pending processing state, issue reporting, lender/member wallet support, and reconciliation references. |
| DM-19 | Imbalance investigation | NEW | Add records for detected discrepancy, source module, expected amount, actual amount, status, assigned investigator, notes, resolution action, adjustment entry, approval trail, and audit linkage. |
| DM-20 | EOD reconciliation | REBUILD | Keep existing reconciliation computation. Add persistent daily sign-off records, discrepancy flags, sign-off blocking rules, resolution references, and admin approval trail. |
| DM-21 | Compassion and default handling | REBUILD | Keep `CompassionAction` and default enforcement foundations. Add restructuring terms, grace periods, penalty waivers, freeze status, reminders, restructuring offer, final write-off state, guarantor charged status, and trust-score impact metadata. |
| DM-22 | Trust scoring | REBUILD | Keep scoring skeleton only. Add PRD member trust weights, tenant trust weights, rating source roles, monthly rating periods, randomized sampling assignments, minimum voting quota, missed-vote lockout, low-rating action state, and tier upgrade/downgrade audit records. |
| DM-23 | Vouch scoring | REBUILD | Keep `SocialVouch` if compatible. Enforce mean-based vouch score with scale/base 10, requester/giver relationship, discount eligibility state, and trust-network visibility metadata. |
| DM-24 | Feedback and support tickets | REBUILD | Keep `FeedbackEntry` and contextual feedback foundations. Add transaction-linked feedback, support ticket status tracking, wallet/loan/payment issue categories, member complaints, system issues, feature requests, homepage concerns, and admin resolution metadata. |
| DM-25 | Audit logs | REBUILD | Keep `AuditLog` and interaction logging foundations. Ensure every module can write actor, tenant, module, entity, action, before/after values, IP/session metadata where available, and cross-tenant visibility for Superadmin. |
| DM-26 | Notifications | REBUILD | Keep `Notification`. Add trigger taxonomy for verification, tenant application, wallet, loan, repayment, voting, defaults, feedback/tickets, reports, announcements, security, system health, and chat events. |
| DM-27 | Email templates | NEW | Add email template records for global announcement broadcaster, security messages, workflow notifications, scheduled reports, and tenant/platform branding. SMS is out of project scope to keep the project no-cost. |
| DM-28 | Chat and community | REBUILD | Keep conversations, participants, messages, attachments, replies, reactions, mentorship connections. Add announcement/bulletin semantics, custom reactions if current reaction model is insufficient, role-specific chat constraints, admin-only Superadmin chat rules, and tenant mentorship feature toggle. UI/UX should be explicitly modeled after Discord/Instagram interaction patterns. |
| DM-29 | Reports | NEW | Add report definition, generated report, recipient, export format, schedule, dispatch status, generated file, and audit linkage for cross-tenant financial reports, tenant performance reports, lender/member reports, and SOA-like outputs. |
| DM-30 | Receipts | NEW | Add receipt records linked to wallet, loan, repayment, fund release, and admin-approved transactions, including receipt number, generated file, status, issued timestamp, and void/reissue audit trail. |
| DM-31 | Backup and recovery | REBUILD | Keep `DecommissionedBackup` download primitive. Add scheduled backup metadata, recovery point, restore request, restore status, affected tenant/user scope, storage location, checksum, and recovery audit trail. |
| DM-32 | AI analytics | NEW | Add AI prompt/config records, snapshot summary records, risk detection sensitivity, processing logs, portfolio summaries, financial tips, suggested investments, and human review metadata. Enforce domain-specific alignment so future AI behavior stays bounded to Agapay analytics/risk/support use cases and cannot be abused outside project intent. |
| DM-33 | Fraud and risk monitoring | NEW | Add cross-tenant fraud signal records for duplicate identities, suspicious transaction patterns, risk thresholds, review status, linked entities, and resolution trail. |
| DM-34 | System health | NEW | Add system health snapshot records for API uptime, queue status, AI processing logs, DB usage per tenant schema, alert state, and Superadmin visibility. |
| DM-35 | Seed and migration policy | REBUILD | Update seeds to create PRD constants, plans, product samples, roles, default calculator configs, default scoring weights, default risk thresholds, default notification templates, and ledger account templates. |
| DM-36 | Test policy coverage | REBUILD | Update business-policy tests to cover PRD constants, membership limit, loan range, fees, penalty cap, guarantor liability, tier mapping, one-active-loan rule, trust/vouch weights, and tenant plan limits. |
