...
export async function getCapitalOversight() {
  ...
  const shareCapital = balances.find(b => b.account_type === 'share_capital') || { total_balance: 0, member_count: 0 };
  const regularSavings = balances.find(b => b.account_type === 'regular_savings') || { total_balance: 0, member_count: 0 };
  ...
  return {
    ...
    totalLiquidAssets: Number(shareCapital.total_balance) + Number(regularSavings.total_balance),
  };
}
...
export async function recordCapitalTransaction(data: {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    // Update balance
    const newBalance = new Decimal(account.balance).add(amount.mul(multiplier));
    if (newBalance.lt(0)) {
    ...
    if (!ledgerAccount) {
      ledgerAccount = await tx.ledgerAccount.create({
        data: {
          tenant_id: tenantId,
          name: data.type === 'share_capital' ? 'Share Capital' : 'Regular Savings',
          code: data.type === 'share_capital' ? 'EQUITY-SC' : 'LIAB-SAV',
