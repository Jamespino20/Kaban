...
export default async function AgapayPintigPage(props: {
  ...
  const totalSavings = savings.reduce(
    (acc: number, curr: { account_type: any; balance: any }) =>
  ...
  const enabledFeatures = tenantIdentity?.tenantSubscription
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              {memberCards.map((item) => (
                <div key={item.label} className="dashboard-card group p-5">
                  <div
                  ...
                  <p
                    className={`text-3xl font-numbers font-bold ${item.color}`}
                  >
                    {item.value}
                  </p>
                </div>
          ...
          >
            {isFeatureEnabled("loans") ? (
              ...
              <RestrictedAccess moduleName="Loan Application" />
            )}
          </TabsContent>
